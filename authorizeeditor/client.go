...
// APIClient manages communication with the PingOne Platform API - Authorize API v2023-06-29
// In most cases there should be only one, shared, APIClient.
type APIClient struct {
	cfg    *Configuration
	common service // Reuse a single struct instead of allocating one for each service on the heap.

	// mu serializes all API calls to prevent concurrent requests.
	// The upstream API persistence layer has race condition issues that require
	// all requests to be executed single-threaded (not in parallel).
	mu sync.Mutex

	// API Services

	AuthorizationVersionsApi *AuthorizationVersionsApiService
...
func NewAPIClient(cfg *Configuration) *APIClient {
	...
	c.AuthorizationVersionsApi = (*AuthorizationVersionsApiService)(&c.common)
